...
hyperparameters are model parameters that cannot be learned directly from a data set during model training.  we can estimate the best values for these values by training many models on resampled data sets and exploring how well all these models perform. This process is called tuning.
