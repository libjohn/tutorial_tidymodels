...
Recipes are designed to help you preprocess your data before training your model. 

- convert qualitative predictors to indicator variables (aka dummy vars)
- transform data to be on a different scale (e.g. `log(var)`)
- transform groups of predictors together
- extract key features from raw vars (e.g. getting the daty of the week out of a data variable)

similar to `formulat()` but brings a wider range of possibilities
...
NOTE:  above - important that our outcome variable (`arr_delay`) for _training_ a logistic regression model is a factor.
...
flight_data_sample <- flight_data %>%
  slice_sample(prop = 0.2)

nrow(flight_data_sample)
nrow(flight_data)
```
...
>  a few specific variables will be important for preprocessing and modeling.
...
1. `arr_delay` is a fctr

2.  two variables that we don’t want to use as predictors in our model, but that we would like to retain as identification variables that can be used to troubleshoot poorly predicted data points: `flight`, and `time_hour` (a date-time)

3. 104 values in `dest` ; 16 distinct `carriers`

```{r}
length(unique(flight_data$dest))
length(unique(flight_data$carrier))
```
...
- test -- used to measure model performance

below uses library(rsample)
...
"A little bit of _feature engineering_ might go a long way to improving our model."
...
(see `step_dummy()` for more info about why dummy vars)
...
See also:  pull_workflow_prepped_recipe()
...
```{r}
flights_pred %>% 
  # filter(arr_delay == "on_time", .pred_on_time < .2)
  # filter(arr_delay == "on_time", .pred_late > .7)
  filter(arr_delay == "late", .pred_on_time > .9)
